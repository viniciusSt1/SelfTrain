...
export default function Explorar({navigation}){
    ...
    async function getExerciciosAgrupados() {
        const snapshot = await firestore().collection('exercices').get();
    
        snapshot.forEach((doc) => {
            const data = doc.data();
            const agrupamento = data.agrupamento;
    
            if (!exerciciosAgrupados[agrupamento]) {
                exerciciosAgrupados[agrupamento] = [];
            }
    
            exerciciosAgrupados[agrupamento].push(data.nome);
        });
                ...
                <View style={styles.screen}>
                    ...
                    {agrupamentos.map((agrupamento, index) => (
                        <Pressable style={style.list_item(isLightMode)} key={index} 
                            onPress={() => navigation.navigate("Treino", {agrupamentos:[agrupamento], titulo:"Explorar", exercicios_agrup:[exerciciosAgrupados[agrupamento] || []], startButton: true})}>
